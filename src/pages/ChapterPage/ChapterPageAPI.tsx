    ...
    <div className="min-h-screen bg-bg-primary text-secondary transition-colors duration-300">
      ...
      <main className="container mx-auto px-4 pt-36 pb-20 max-w-4xl">
        ...
        <section className="mb-16">
            <h3 className="text-xl font-bold text-primary mb-6">Comments</h3>
            ...
            <form onSubmit={handleCommentSubmit} className="mb-8">
                <textarea
                    ...
                    disabled={!user}
                    className="w-full bg-surface border border-border rounded-lg p-4 text-primary placeholder-muted focus:border-neon-gold focus:ring-1 focus:ring-neon-gold transition-all resize-none h-24"
                />
                <div className="flex justify-end mt-2">
                    <button
                        type="submit"
                        disabled={!user || submittingComment || !newComment.trim()}
                        className="bg-neon-gold text-black px-6 py-2 rounded-lg font-bold hover:bg-yellow-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
            ...
            <div className="space-y-6">
                {comments.map((comment: any) => (
                    <div key={comment.id} className="bg-surface/50 p-4 rounded-lg border border-border">
                        <div className="flex justify-between items-start mb-2">
                            <span className="font-bold text-neon-gold">{comment.user?.name || 'User'}</span>
